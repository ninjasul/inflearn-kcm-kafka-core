...
public class CommitAsyncConsumer {
    public static final Logger log = LoggerFactory.getLogger(CommitAsyncConsumer.class);

    public static void main(String[] args) {
        Properties props = buildProperties();

        String topicName = "commit-async-topic";
    ...
    private static Properties buildProperties() {
        ...
        props.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "commit-async-group-01");
    ...
    private static void consume(KafkaConsumer<String, String> kafkaConsumer) {
        polllWithCommitAsync(kafkaConsumer);
    }

    private static void polllWithCommitAsync(KafkaConsumer<String, String> kafkaConsumer) {
        int loopCnt = 1;
        ...
        try {
            while (true) {
                ...
                try {
                    kafkaConsumer.commitAsync((offsets, exception) -> {
                        if (exception != null) {
                            log.error("offsets {} are not completed, error: {}", offsets, exception);
                        }
                    });
                    log.info("commitAsync() has been invoked.");
            ...
            log.info("Trying commitSync() before closing.");
