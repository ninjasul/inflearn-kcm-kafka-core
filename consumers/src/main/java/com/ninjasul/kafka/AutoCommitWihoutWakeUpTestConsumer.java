...
public class AutoCommitWihoutWakeUpTestConsumer {
    public static final Logger log = LoggerFactory.getLogger(AutoCommitWihoutWakeUpTestConsumer.class);
    ...
    private static Properties buildProperties() {
        ...
        props.setProperty(ConsumerConfig.GROUP_ID_CONFIG, "auto-commit-group-01");
    ...
    private static void pollAutoCommit(KafkaConsumer<String, String> kafkaConsumer) {
        ...
        try {
            while (true) {
                ...
                try {
                    log.info("The main thread is sleeping for {} ms during while loop.", 10000);
                    Thread.sleep(10000L);
                } catch (InterruptedException e) {
                    log.error("", e);
                }
